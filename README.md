...
## Phase 3: Interactive Empire Map

Phase 3 adds the client/server empire-map dashboard:

- Press **M** (configurable in Controls) or use the **Map** button in the player inventory.
- Drag to pan, scroll to zoom, left-click a claimed chunk for details, and right-click to clear selection.
- The server sends the player's own state, only directly scouted foreign border chunks, and future war-visible states.
- Horizontal run-length compression reduces large territory payloads before network transmission.
- State and province borders are precomputed when the payload arrives instead of every frame.
- While the screen is open, it refreshes every two seconds so border changes and breach markers stay current.
- Province labels use the City Altar position when available and fall back to a chunk center-of-mass calculation.
- Recent successful hostile border flips appear as breach markers and send chat warnings to the defending state.

The formal war-relation visibility hook is intentionally left in `MapDataService#getWarVisibleStateIds` for the later army/war phases.
